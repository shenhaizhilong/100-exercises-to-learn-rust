use crate::TicketNewError::{DescriptionErr, TitleErr};
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TitleErr { msg: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TitleErr { msg: "Title cannot be longer than 50 characters".to_string() });
        }
        if description.is_empty() {
            return Err(DescriptionErr { msg: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(DescriptionErr { msg: "Description cannot be longer than 500 characters".to_string() });
        }
